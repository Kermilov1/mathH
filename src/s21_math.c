...
long double s21_pow(double a,
                    double b) {  // возведение в степень натурального числа
...
long double s21_floor(double a) {  // округление в меньшую сторону
  long double x = a;
  return x - (int)x > 0 ? x - (x - (int)x) : x - (int)x < 0 ? (int)x - 1 : x;
}

int s21_abs(int a) {  // модуль целого числа
  if (a < 0) a = -a;
  return a;
}

long double s21_sqrt(double a) {  // квадратный корень числа методом Ньютона
  double t = 0, sqrt = a / 2;
  if (a > 0) {
    while ((t - sqrt) != 0) {
      t = sqrt;
...
long double s21_fabs(double a) {  // модуль дробного числа
  if (a < 0.0) a = -a;
  return a;
}

long double s21_exp(double a) {  // вычисление экспоненты
  long double res, t;
  long double b = a;
  int znak = 0;
  if (b < 0) {
...
long double s21_fmod(
    double a,
    double b) {  // вычисление остатка от деления нацело для чисел с плав.точкой
...
long double s21_sin(double a) {  // Ряд Тейлора, см.также ряд Маклорена
...
long double s21_tan(double a) {  // тангенс по формуле
  return s21_sin(a) / s21_cos(a);
}
long double s21_asin(double a) {  // арксинус, ряд Маклорена
  int metka = 0;
  if (a < 0 && a != -1) {
    a = a * -1;
    metka = 1;
...
long double s21_acos(double a) {  // арккосинус по отношению к арксинусу
  long double res = 0;
  if (a < 1 && a > -1) {
...
long double s21_atan(double a) {  // арктангенс по отношению к арксинусу
